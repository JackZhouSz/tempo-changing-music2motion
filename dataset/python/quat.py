"""
Quaternion utilities used for SMPL rotation processing.

This is a direct adaptation of the internal helper module used in the
original JoruriPuppet preprocessing scripts. Only NumPy is required.
"""
...
def log(x, eps=1e-5):
    length_ = np.sqrt(np.sum(np.square(x[..., 1:]), axis=-1))[..., None]
    halfangle = np.where(
        length_ < eps,
        np.ones_like(length_),
        np.arctan2(length_, x[..., 0:1]) / length_,
    )
...
def exp(x, eps=1e-5):
    ...
    s = np.where(
        halfangle < eps, np.ones_like(halfangle), np.sinc(halfangle / np.pi)
    )
...
def fk_vel(lrot, lpos, lvel, lang, parents):
    ...
    for i in range(1, len(parents)):
        ...
        gv.append(
            ...
            + _fast_cross(
                ga[parents[i]],
                mul_vec(gr[parents[i]], lpos[..., i : i + 1, :]),
            )
            + gv[parents[i]]
        )
...
def to_xform(x):
    ...
    return np.concatenate(
        [
            np.concatenate(
                [1.0 - (yy + zz), xy - wz, xz + wy], axis=-1
            )[..., None, :],
            np.concatenate(
                [xy + wz, 1.0 - (xx + zz), yz - wx], axis=-1
            )[..., None, :],
            np.concatenate(
                [xz - wy, yz + wx, 1.0 - (xx + yy)], axis=-1
            )[..., None, :],
        ],
        axis=-2,
    )
...
def to_axis_angle(x, eps=1e-5):
    ...
    sin_half_angle_over_angle[small_angle] = 0.5 - (
        angle[small_angle] * angle[small_angle]
    ) / 48
